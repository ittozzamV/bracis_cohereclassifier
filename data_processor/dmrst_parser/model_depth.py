...
from .module import (DecoderRNN, EncoderRNN, LabelClassifier, PointerAtten,
                     Segmenter)
...
class ParsingNet(nn.Module):
  ...
  def __init__(self,
    ...
    self.encoder = EncoderRNN(language_model,
                              word_dim,
                              hidden_size,
    ...
    self.decoder = DecoderRNN(decoder_input_size, hidden_size, rnn_layers,
                              dropout_d)
    self.pointer = PointerAtten(atten_model, hidden_size)
    self.getlabel = LabelClassifier(classifier_input_size,
  ...
  def TrainingLoss(self, input_sentence, EDU_breaks, LabelIndex, ParsingIndex,
                   DecoderInputIndex, ParentsIndex, SiblingIndex):

    # Obtain encoder outputs and last hidden states
    EncoderOutputs, Last_Hiddenstates, total_edu_loss, _ = self.encoder(
        input_sentence, EDU_breaks)
    ...
    for i in range(batch_size):
      ...
      if len(EDU_breaks[i]) == 1:
        ...
        Loss_label_batch = Loss_label_batch + Label_LossFunction(
            log_relation_weights, cur_LabelIndex)
        ...
        for j in range(len(cur_DecoderInputIndex)):
          ...
          if stacks[-1] != '__StackRoot__':
            ...
            if len(stack_head) < 3:
              ...
              cur_decoder_input = torch.mean(cur_EncoderOutputs[stack_head],
                                             keepdim=True,
                                             dim=0).unsqueeze(0)
              ...
              _, log_relation_weights = self.getlabel(input_left, input_right)
              Loss_label_batch = Loss_label_batch + Label_LossFunction(
                  log_relation_weights, cur_LabelIndex[j].unsqueeze(0))
              ...
              _, log_atten_weights = self.pointer(
                  cur_EncoderOutputs[stack_head[:-1]],
              ...
              if config.average_edu_level is True:
                input_left = torch.mean(
                    cur_EncoderOutputs[stack_head[0]:cur_ParsingIndex[j] +
                                       1, :],
                    keepdim=True,
                    dim=0)
                input_right = torch.mean(
                    cur_EncoderOutputs[cur_ParsingIndex[j] + 1:stack_head[-1] +
                                       1, :],
                    keepdim=True,
                    dim=0)
              else:
                input_left = cur_EncoderOutputs[cur_ParsingIndex[j]].unsqueeze(
                    0)
                input_right = cur_EncoderOutputs[stack_head[-1]].unsqueeze(0)
              ...
              # Stacks stuff
              stack_left = stack_head[:(cur_ParsingIndex[j] - stack_head[0] +
                                        1)]
              stack_right = stack_head[(cur_ParsingIndex[j] - stack_head[0] +
                                        1):]
              del stacks[-1]
              Loop_label_batch = Loop_label_batch + 1
  ...
  def TestingLoss(self, input_sentence, input_EDU_breaks, LabelIndex,
                  ParsingIndex, GenerateTree, use_pred_segmentation):
    '''
            Input:
        ...
        '''
    # Obtain encoder outputs and last hidden states
    EncoderOutputs, Last_Hiddenstates, _, predict_edu_breaks = self.encoder(
        input_sentence, input_EDU_breaks, is_test=use_pred_segmentation)
    ...
    for i in range(len(EDU_breaks)):
      ...
      if len(EDU_breaks[i]) == 1:
        ...
        input_left = cur_EncoderOutputs[0].unsqueeze(0)
        input_right = cur_EncoderOutputs[1].unsqueeze(0)
        relation_weights, log_relation_weights = self.getlabel(
        ...
        if GenerateTree:
          ...
          Nuclearity_left, Nuclearity_right, Relation_left, Relation_right = get_RelationAndNucleus(
              LabelPredict)
        ...
        while stacks[-1] != '__StackRoot__':
          ...
          if len(stack_head) < 3:
            ...
            if config.average_edu_level is True:
              input_left = torch.mean(
                  cur_EncoderOutputs[stack_head[0]:TreePredict + 1, :],
                  keepdim=True,
                  dim=0)
              input_right = torch.mean(
                  cur_EncoderOutputs[TreePredict + 1:stack_head[-1] + 1, :],
                  keepdim=True,
                  dim=0)
            else:
    ...
    return Loss_tree_batch, Loss_label_batch, (
        SPAN_batch if GenerateTree else None), (merged_label_gold,
                                                merged_label_pred), EDU_breaks
