...
class EncoderRNN(nn.Module):
  ...
  def __init__(self,
    ...
    self.doc_gru_enc = nn.GRU(word_dim,
                              int(word_dim / 2),
                              num_layers=2,
                              batch_first=True,
                              dropout=0.2,
  ...
  def forward(self, input_sentence, EDU_breaks, is_test=False):
    ...
    for i in range(len(input_sentence)):
      ...
      for tmp_step in range(slide_steps):
        if tmp_step == 0:
          one_win_res = self.language_model(
              bert_token_ids[:, :500])[0][:, :window_size, :]
          window_embed_list.append(one_win_res)
        elif tmp_step == slide_steps - 1:
          one_win_res = self.language_model(
              bert_token_ids[:,
                             -((sequence_length -
                                (window_size * tmp_step)) + 200):])[0][:,
                                                                       200:, :]
          window_embed_list.append(one_win_res)
        else:
          one_win_res = self.language_model(
              bert_token_ids[:, (window_size * tmp_step -
                                 100):(window_size * (tmp_step + 1) +
                                       100)])[0][:, 100:400, :]
          window_embed_list.append(one_win_res)
      ...
      if is_test:
        ...
        seg_loss = self.segmenter.train_segment_loss(embeddings.squeeze(),
                                                     cur_edu_break)
      ...
      for tmp_i in range(len(tmp_break_list) - 1):
        ...
        tmp_average_list.append(
            torch.mean(embeddings[tmp_break_list[tmp_i]:tmp_break_list[tmp_i +
                                                                       1], :],
                       dim=0,
                       keepdim=True))
      ...
      if config.add_first_and_last is True:
        ...
        for tmp_i in range(len(tmp_break_list) - 1):
          ...
          last_words.append(embeddings[tmp_break_list[tmp_i + 1] -
                                       1].unsqueeze(dim=0))

        outputs = torch.cat(
            (outputs, torch.cat(first_words, dim=0).unsqueeze(dim=0),
    ...
    for output in tem_outputs:
      ...
      all_outputs.append(
          torch.cat([
              output,
              torch.zeros(1, max_edu_break_num - cur_break_num,
                          self.word_dim).cuda()
          ],
                    dim=1))
  ...
  def GetEDURepresentation(self, input_sentence):
    ...
    bert_token_ids = [
        self.bert_tokenizer.convert_tokens_to_ids(v) + [
            5,
        ] * (tmp_max_token_num - len(v)) for k, v in enumerate(input_sentence)
    ]
    bert_token_ids = torch.LongTensor(bert_token_ids).cuda()
  ...
  def __init__(self,
    ...
    if bias:
      self.weight_bilateral = nn.Bilinear(classifier_hidden_size,
                                          classifier_hidden_size, classes_label)
    else:
      self.weight_bilateral = nn.Bilinear(classifier_hidden_size,
                                          classifier_hidden_size,
                                          classes_label,
...
class Segmenter_pointer(nn.Module):
  ...
  def __init__(self,
    ...
    self.encoder = nn.GRU(hidden_size,
                          int(hidden_size / 2),
                          num_layers=1,
                          batch_first=True,
                          dropout=0.2,
  ...
  def train_segment_loss(self, word_embeddings, edu_breaks):
    ...
    for step, start_index in enumerate(edu_breaks[:-1]):
      ...
      cur_ground_index = torch.tensor([edu_breaks[step + 1] - start_index
                                      ]).cuda()
      total_loss = total_loss + self.loss_function(log_atten_weights,
                                                   cur_ground_index)
...
class Segmenter(nn.Module):
  ...
  def train_segment_loss(self, word_embeddings, edu_breaks):
    ...
    if config.if_edu_start_loss:
      total_loss = self.loss_function(outputs,
                                      edu_break_target) + self.loss_function(
                                          start_outputs, edu_start_target)
    else:
